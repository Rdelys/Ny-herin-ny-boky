{{-- ============ FOOTER ============ --}}
<footer>
    <div class="wrap">
        <div class="foot-top">
            <div class="foot-brand">
                <img src="{{ asset('logo.png') }}" alt="Ny Herin'ny Boky">
                <span>Ny Herin'ny Boky</span>
            </div>
            <ul class="foot-links">
                <li><a href="https://www.ny-erin-ny-boky.com/a-propos">{{ __('home.footer_about') }}</a></li>
                <li><a href="https://www.ny-erin-ny-boky.com/confidentialite">{{ __('home.footer_privacy') }}</a></li>
                <li><a href="https://www.ny-erin-ny-boky.com/cgv">{{ __('home.footer_terms') }}</a></li>
            </ul>
        </div>
        <p class="foot-bottom">{{ __('home.footer_copyright') }}</p>
    </div>
</footer>